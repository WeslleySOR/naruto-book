...
export function Ninja({ onOpenEditNinjaModal }: NinjaProps) {
	...
	const deleteCharacter = async () => {
		if (
			window.confirm("Deseja realmente apagar o personagem do banco de dados?")
		) {
				...
				<>
					<CharacterHeader>
						...
						<div className="div-info">
							<h1>
								Nome:
								<span>
									{characters.find((char) => char.id === id)?.info.clan !==
									"Desconhecido"
										? characters.find((char) => char.id === id)?.info.clan
										: ""}{" "}
								</span>
								<span>
									{characters.find((char) => char.id === id)?.info.name}
								</span>
							</h1>
							<h1>
								Clan:{" "}
							...
							<h1>
								Patente ninja:{" "}
								<span>
									{characters.find((char) => char.id === id)?.info.level}
								</span>
							</h1>
						</div>
					</CharacterHeader>
					<CharacterContent>
						<div>
							<h1>História</h1>
							<ButtonGroup>
								<button onClick={onOpenEditNinjaModal}>
									<span>Editar</span>
									<BsPencilSquare />
								</button>
